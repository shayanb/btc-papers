...
%
Throughout our walkthroughs, we identified multiple occurrences of highly specialized and technical language in the Bitcoin clients. Several clients (x, y and z) had messages that referenced the client being ``out of sync'', meaning that a full copy of the blockchain was pending download completion. Another message reported the number of peers to which the client was currently connected as simply ``x connections to the network''. The decentralized architecture of Bitcoin benefits from a higher number of connections, but this is hard for the user to control, and has little significance to the functionality of the client as long as the number is greater than zero. Other clients communicated this network connectivity issue as simply ``online/offline'' which is a simpler abstraction of the concept. 
