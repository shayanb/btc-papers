...
To send funds from a Bitcoin address that has been stored on a paper wallet, the user must import the private key into one of the wallet clients such as \armory or \bitcoinclient. Importing the private key may require taking a picture of the QR code or typing in the private key manually, depending on the client. Once the key has been imported, the user can proceed to spend Bitcoins as instructed by the particular client. We note that if the client returns change to newly generated addresses, and the user does not spend the full amount on the paper wallet, subsequently importing the paper wallet onto a different device will likely display no funds. 
